...
struct DecimalOpHelpers
{
    ...
    static std::vector<UInt8> divide(const std::vector<UInt8> & number, const Int512 & divisor)
    {
    ...
    static std::vector<UInt8> toDigits(Int512 x)
    {
        std::vector<UInt8> result;
        if (x >= 10)
    ...
    static UInt512 fromDigits(const std::vector<UInt8> & digits)
    {
        Int512 result = 0;
        Int512 scale = 0;
