    ...
    DataTypePtr & out_inferred_type, std::optional<GeoColumnMetadata> geo_metadata) const
{
    ...
    else if (logical.__isset.DECIMAL || converted == CONV::DECIMAL)
    {
        ...
        if (precision > 154)
            throw Exception(
                ErrorCodes::INCORRECT_DATA,
                "Parquet Decimal precision {} exceeds the maximum supported ClickHouse Decimal precision 154",
                precision);
    ...
    switch (type)
    {
        ...
        case parq::Type::FIXED_LEN_BYTE_ARRAY:
        {
            if (type_hint)
            {
                ...
                /// ConverterDecimal<Decimal512> writes big-endian bytes; decode back to little-endian.
